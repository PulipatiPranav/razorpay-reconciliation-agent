...
**Status: Phases 1–4 complete. Layer 3 is built and unit-tested but has not yet
been run live — no API credentials in this environment, so every measured number
below is Layers 1–2. Phase 5 (audit-trail view) pending.**
...
Everything below is generated by `make eval` — commit `9f4689b`, confidence threshold 0.70, Layer 3 mode `replay`. Do not edit by hand; `recon eval --check` fails when this block is stale.
...
`reports/results.md`, and every LLM call in `logs/llm_calls.jsonl`.
