...
def build_report(context: ReportContext) -> str:
    ...
    parts: list[str] = [
        "## Results",
        "",
        "Everything below is generated by `make eval` — confidence threshold "
        f"{context.threshold:.2f}, Layer 3 mode `{context.llm_mode}`, corpus identified "
        "by the config hash in the Corpus table below. Do not edit by hand; "
        "`make eval-check` fails when this block is stale.",
        "",
    ]
